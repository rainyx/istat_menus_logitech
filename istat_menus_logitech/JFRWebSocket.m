...
static NSString *const headerWSExtNamae        = @"Sec-WebSocket-Extensions";
...
- (void)createHTTPRequest {
    ...
    NSString *hostName = [NSString stringWithFormat:@"%@:%@",self.url.host,port];
    NSString *key = [self generateWebSocketKey];
    ...
    CFHTTPMessageSetHeaderFieldValue(urlRequest,
                                     (__bridge CFStringRef)headerWSExtNamae,
                                     (__bridge CFStringRef)@"permessage-deflate; client_max_window_bits");
    CFHTTPMessageSetHeaderFieldValue(urlRequest,
                                     (__bridge CFStringRef)@"User-Agent",
                                     (__bridge CFStringRef)@"Python/3.8 websockets/10.2");
...
//                                     (__bridge CFStringRef)origin);
    ...
    NSDictionary *headerDic = (__bridge NSDictionary *)CFHTTPMessageCopyAllHeaderFields(urlRequest);
